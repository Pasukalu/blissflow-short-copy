...
class GenerateRequest(BaseModel):
    product_url: str
    product_name: str
    copy_type: str = "short"  # short / long / video / all
...
async def generate_copy(req: GenerateRequest):
    ...
    # 單個類型
    if req.copy_type != "all":
        if req.copy_type not in COPY_TYPES:
            raise HTTPException(status_code=400, detail=f"Unknown copy_type: {req.copy_type}")

        config = COPY_TYPES[req.copy_type]
    ...
    import asyncio
    tasks = []
    types_order = ["short", "long", "video"]

    for ct in types_order:
        config = COPY_TYPES[ct]
        tasks.append(call_dashscope(config["prompt"], user_message))

    results = await asyncio.gather(*tasks, return_exceptions=True)

    combined = {}
    errors = []
    for i, ct in enumerate(types_order):
        r = results[i]
        if isinstance(r, Exception):
            errors.append(f"{COPY_TYPES[ct]['name']}: {str(r)}")
            combined[ct] = {"content": f"❌ 生成失敗: {r}", "error": True}
        else:
            combined[ct] = {"content": r["content"], "error": False}
